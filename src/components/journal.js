...
export default function journal({ item , showModal ,deleteItem }) {
    return (
        <div>
            <div  className="mx-5 my-2 flex w-90 flex-col justify-center bg-white rounded-2xl shadow-xl shadow-gray-300">
                <div className="p-4 flex-row mx-1 my-1">
                    <div className="flex justify-between"> 
                    <span className="flex justify-start" > {item.date}</span>  
                    <AiOutlineDelete onClick={()=> deleteItem(item.id)}className='flex justify-end text-2xl text-red-500' />        
                    </div>
